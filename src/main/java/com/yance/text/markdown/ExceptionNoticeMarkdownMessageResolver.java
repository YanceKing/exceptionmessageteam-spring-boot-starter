...
public class ExceptionNoticeMarkdownMessageResolver implements ExceptionNoticeResolver {
	...
	protected String resolve(HttpExceptionNotice exceptionNotice) {
		...
		String markdown = SimpleMarkdownBuilder.create().title(SimpleMarkdownBuilder.bold(title), 1)
				...
				.text(httpExceptionNotice.getCreateTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),
						true)
	...
	public boolean support(Class<? extends PromethuesNotice> clazz, INoticeSendComponent noticeSendComponent) {
		return ExceptionNotice.class.isAssignableFrom(clazz) && noticeSendComponent.name().equals("dingding");
	}
